...
import { Mail, Linkedin, Github, Instagram, Youtube, Send, Terminal, ShieldCheck, Globe } from 'lucide-react';
            ...
            {socialLinks.map((link) => (
              ...
              >
                <div className="flex items-center gap-5 relative z-10">
                  <div className="p-3 bg-bg-editor rounded-lg border border-border-color group-hover:border-white/10 transition-colors" style={{ color: link.color }}>
                    <link.icon size={20} />
                  </div>
          ...
          <AnimatePresence mode="wait">
            {!submitted ? (
              ...
              >
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    type="text"
                    placeholder="string"
                    className="bg-bg-editor border border-border-color rounded-lg w-full px-4 py-3 text-[14px] font-mono text-text-primary focus:outline-none focus:border-[#00e5cc] transition-colors placeholder:opacity-20"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    rows={4}
                    placeholder="string"
                    className="bg-bg-editor border border-border-color rounded-lg w-full px-4 py-3 text-[14px] font-mono text-text-primary focus:outline-none focus:border-[#00e5cc] transition-colors placeholder:opacity-20 resize-none"
                  />
                </div>
                ...
                <button 
                  disabled={loading}
                  type="submit"
                  className="bg-[#00e5cc] text-[#1e1e1e] font-bold font-mono text-[14px] py-4 rounded-xl hover:bg-[#00b8a9] transition-all flex items-center justify-center gap-3 shadow-[0_4px_20px_rgba(0,229,204,0.2)] disabled:opacity-50 disabled:cursor-not-allowed group"
                >
                  {loading ? (
              ...
              >
                ...
                <p className="text-text-muted text-[13px] font-mono max-w-[250px] leading-relaxed">
                  Your signal has been received. I will respond via encrypted channel soon.
                </p>
                <button 
                  onClick={() => setSubmitted(false)}
                  className="mt-8 text-text-cyan hover:underline text-[13px] font-mono flex items-center gap-2"
                >
                  {"← run_new_session( )"}
                </button>
              </motion.div>
